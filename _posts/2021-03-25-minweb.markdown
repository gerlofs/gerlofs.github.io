...
layout: front
title: "A bloat ride"
cover: /assets/img/posts/minweb/wiby.jpg
tags: blog
...
<h2 align="center">A bloat ride through a day of HN articles</h2>
...
that is, the size of web pages - such as Dan Luu's article on [restrictive web bloat for rural internet users](https://danluu.com/web-bloat)\*<sub>0</sub> and Maciej Cegłowski's 
...
to render a webpage fully is over [10 seconds](https://backlinko.com/page-speed-stats) on desktop and double that on mobile clients. Unsurprisingly, the average connection 
...
What does this all culminate to? An averagely shitty experience accessing the modern web. The *average* modern website is optimised for the *average* user\*<sub>2</sub>, this works in theory as internet speeds 
...
negatively impact your traffic and, thus, conversions - marketing lingo for getting lurker plebians to buy your wares. The article, which, without edits to the spacing, can fit comfortably on three sides of A4, tells
...
(CTRL+ALT+R on Firefox, if you are somehow unaware). The obvious reason for this is the infographic taken from [Skilled.co](https://skilled.co/resources/speed-affects-website-infographic/) which,
 as far as I'm concerned, could be replaced with a link - this would further cut the page weight (without Javascript) to less than 1MB. This might *sound* like an improvement but this is still 846 bytes per
word. To put this kind of page bloat into perspective, again, borrowing from Maciej, David Foster Wallace's *Infinite Jest*, one of the longest English novels ever written, has 543,709 words, and is still
lighter (~1.5MB in EPUB, ~3MB in PDF) that this webpage. 
...
>3: Replace embedded scripts (such as the embedded YouTube player) with reasonable alternatives if removing it from the page would not significantly alter the quality of the page. For example,
there's no reason why any of the above examples could not replace the embedded player with a static string or image linking (in another tab) to the YouTube video.

>4: Stop using blogging platforms that allow you to do none of the above. Setting up a website is easy, there are guides on the internet about HTML and CSS, this is all you need to make a 
personal blog or simplistic site - using templating software or blog platforms is tempting but can result in pointlessly bloated websites\*<sub>6</sub>.

>5: You can't avoid bloatware sites, so get NoScript (or a similar JS blocker) and an ad-blocker.

>6: If you find a blog you like, add it to an [RSS feed](https://www.thesitewizard.com/faqs/howtoreadsitefeeds.shtml) and build up a subscription portfolio of small, informative sites.

There is no happy ending to this ride. Unless search engines started allowing users to search by page weight, I don't really forsee an easy solution to the continuing problem of web bloat.

<h2>Appendix/Footnotes:</h2>

\*<sub>0</sub> Note that one of Dan's examples is [codinghorror](https://blog.codinghorror.com/) (no pun intended, I guess), one of the most egregious examples of web bloat in a blog site, 45.15MB!
